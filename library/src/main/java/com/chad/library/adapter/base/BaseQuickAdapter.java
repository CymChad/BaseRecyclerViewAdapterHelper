...
public abstract class BaseQuickAdapter<T, K extends BaseViewHolder> extends RecyclerView.Adapter<K> {
     ...
     * change listData
     */
    public void setData(@IntRange(from = 0) int positionStart, @NonNull Collection<? extends T> newDatas) {
        setData(positionStart, newDatas.size(), newDatas);
    }

    /**
     * change listData
     */
    public void setData(@IntRange(from = 0) int positionStart,
            int itemCount, @NonNull Collection<? extends T> newDatas) {
        int i = 0;
        int size = mData.size();
        for (T data : newDatas) {
            if (positionStart + i >= size || i >= itemCount) {
                break;
            }
            mData.set(positionStart + i++, data);
        }
        notifyItemRangeChanged(positionStart + getHeaderLayoutCount(), i);
    }
